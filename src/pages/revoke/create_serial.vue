<template>
  <q-page padding class="row justify-center">
    <div style="width: 700px; max-width: 90vw;">
      <q-input v-model="serial" @input="submitForm" float-label="ระบุ Serial Number"  :after="[{icon: 'camera_alt', handler () {scanBarcode()}}]" />
  ...
  export default {
    ...
    computed:{
      ...
      methods: {
        ...
        submitForm: function () {
          this.$axiosInstance.post('/revoke/serial/', this.serial)
            .then(response => {
              this.$q.notify({
                type: 'positive',
        ...
        promptDialog: function () {
          this.$q.dialog({
            title: 'ข้อมูลอุปกรณ์',
            message: 'มีการเก็บอุปกรณ์หรือไม่?',
            ok: 'มี',
            cancel: 'ไม่มี'
          }).then(() => {
